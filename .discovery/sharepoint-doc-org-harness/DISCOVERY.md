...
Research deliverable: OSS adopt/wrap/reject table that measurably reduces
harness code we would otherwise write.
...
- Research must complete an **OSS reuse survey** (high-star, maintained GitHub) and prefer wrap/integrate over greenfield.
...
### OSS reuse mandate (Stage 3 / research obligation)

Research **must** survey well-maintained, high-star open-source GitHub projects
before proposing net-new code. Prefer integrate/wrap over rewrite. Seed shortlist
from external scan 2026-08-12 (stars via GitHub API that day; verify at research
time). Not an adoption decision — candidates for RESEARCH.md evaluate/reject.

| Area | Repo | Stars (approx) | Why it may shrink our build |
|------|------|----------------|-----------------------------|
| Doc convert / extract (local) | [docling-project/docling](https://github.com/docling-project/docling) | ~65k | PDF/Office → structured text/JSON for local-LLM classify; already in PLX skills |
| OCR layer | [ocrmypdf/OCRmyPDF](https://github.com/ocrmypdf/OCRmyPDF) | ~34k | Searchable PDF text layer for scans |
| DMS / ingest patterns | [paperless-ngx/paperless-ngx](https://github.com/paperless-ngx/paperless-ngx) | ~44k | Ingest, tag, classify, archive patterns (SharePoint remains SoT — do not replace SoT unless research argues) |
| Dedup | [pkolaczk/fclones](https://github.com/pkolaczk/fclones) | ~2.9k | Fast content-hash duplicate groups; JSON/CSV output for harness |
| Dedup (alt) | [pauldreik/rdfind](https://github.com/pauldreik/rdfind) | ~1.3k | Mature content-hash duplicate finder |
| Graph API (Python) | [microsoftgraph/msgraph-sdk-python](https://github.com/microsoftgraph/msgraph-sdk-python) | ~0.6k | Official Graph SDK for SharePoint + mail |
| Graph API (.NET) | [microsoftgraph/msgraph-sdk-dotnet](https://github.com/microsoftgraph/msgraph-sdk-dotnet) | ~0.8k | Official .NET Graph SDK if Windows-native host wins |
| FS abstraction | [fsspec/filesystem_spec](https://github.com/fsspec/filesystem_spec) | ~1.3k | Filesystem-like I/O patterns; Graph drive adapters exist in ecosystem |
| Metadata extract | [apache/tika](https://github.com/apache/tika) | ~4.0k | Broad MIME/metadata extraction |
| Graph DB | [neo4j/neo4j](https://github.com/neo4j/neo4j) | ~17k | Provenance / where-did-file-go graph |
| Graph DB (lighter) | [memgraph/memgraph](https://github.com/memgraph/memgraph) | ~4.3k | Cypher-compatible, lower-footprint candidate |
| Local vectors (optional) | [qdrant/qdrant](https://github.com/qdrant/qdrant) / [lancedb/lancedb](https://github.com/lancedb/lancedb) | ~34k / ~11k | Semantic search over filings if needed; must stay local |
| Tagging UX patterns | [tagspaces/tagspaces](https://github.com/tagspaces/tagspaces) | ~5.2k | Offline tagging / meta conventions (ideas, not SoT) |
| Bookmark/AI tag patterns | [karakeep-app/karakeep](https://github.com/karakeep-app/karakeep) | ~28k | Self-host AI tagging + full-text patterns |

**Low-star but pattern-relevant (email):** Graph + local-LLM mail sorters exist
but lack star/maintenance signals — research may mine ideas, not depend on them
as foundations.

**Reuse rule:** RESEARCH.md must include an adopt / wrap / reject table for each
row above (plus any better finds), with license, host OS fit (Windows VTA +
laptop), SharePoint SoT fit, and local-model-only fit.
...
- Local model contract: `litellm/config.yaml` (Qwen Dell lanes + Ornith/Qwen Spark
  lanes via proxy).
- External OSS scan 2026-08-12 via GitHub API + web search (Docling, Paperless-ngx,
  OCRmyPDF, fclones, Graph SDKs, Neo4j/Memgraph, Qdrant/LanceDB, TagSpaces,
  Karakeep). Star counts are point-in-time.
...
- Round-1 feedback (2026-08-12): Vince: "This is good" on Q1–Q5, plus modify —
  include external research of well-maintained high-star GitHub OSS to minimize
  custom code. Seeded OSS shortlist into ledger; mint new candidate.
- Round-2 (2026-08-12): Vince approved OSS reuse + SharePoint SoT; deferred
  shortlist curation to agent. Gate approved for research+plan (execution still
  outstanding). Handed off to project-researcher; RESEARCH.md drafted.

## Handoff

- Target mode: research+plan+execute
- Candidate digest: sha256:0f053485230e80ef881c98f39430074a4e1e676412cf7381547f19c747e97902
- Review round: `.discovery/sharepoint-doc-org-harness/review/round-2.md`
- Required approvers satisfied: cos@petrasoap.com (AUTH-OWNER@1)
- Spine skill: `project-researcher` → `.orchestrator/sharepoint-doc-org-harness/RESEARCH.md`,
  then `project-orchestrator` Stages 1-2 (spec gate), then Stage 3+ only after
  separate execution authorization
- Research must deliver OSS adopt/wrap/reject table (seeded shortlist in Answers;
  shortlist curation deferred to agent)
- Outstanding gates: project-orchestrator Stage 2 spec approval; **execution
  authorization still outstanding** (not granted by discovery review)
