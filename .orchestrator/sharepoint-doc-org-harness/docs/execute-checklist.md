...
7. [x] Unique `09_Archive` + Petra root drain 2026-08-18 run `8656c0dce43d45a9b980378487202ee4` (1614 moved). Leftover VincePersonal roots folded run `832a481bbf044d09be48e78b27f73a0f` (442 moved); empty leftover tops removed. Old Petra sources hidden (`hide-petra-sources.ps1 -Apply`). Vince Personal not hidden. `cursor-inbox` and `Lattice-Peptides` remain as excluded code trees, hidden.

Inference stays on Dell LiteLLM `local-driver` / `local-coder`. Paid hosts stay forbidden.

Open follow-ups: TASK-1063 (every-4h after daily proof), TASK-1064 (mail remainder), TASK-1068 (fold leftover roots, drain archive, hide Petra). TASK-1065 is done.
