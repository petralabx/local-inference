...
1. [ ] Five-file smoke in VincePersonal `00_Inbox` (not Desktop). Journal moves. `harness reverse` undoes them.
2. [ ] VTA redirect: `scripts/redirect-known-folders.ps1 -DryRun`, then `-Apply` only while Vince is present.
