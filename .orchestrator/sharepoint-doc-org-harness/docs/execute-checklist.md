...
4. [ ] Full drain of the Petra map (`config/drain_map.yaml`). Unique files only. Hash copies skip. Started 2026-08-16 on VTA (`harness drain`). Numbered tree except `07_Admin` is drained (about 4,006 unique moved; hash copies left in place; 3 secrets skipped). Still open: `07_Admin`, Desktop, CursorInbox, Vince Backup, OLD LAPTOP FILES, leftover laptop Desktop/Documents/Downloads.
