...
11. [ ] On VTA only: `harness sync-audit --dry-run` — inspect `data/reports/sync-audit.json` for local-only / server-only / path mismatches. Cloud VMs cannot see the sync root.
